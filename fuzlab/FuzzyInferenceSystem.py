...
class FuzzyInferenceSystem:
    def __init__(self):
        ...
        self.Outputs    = []
        self.Rules      = []
        self.WablOptimizm    = 0.5      # optimism parameter for WABL
        self.WablDegrees     = 21       # number of discrete level sets for WABL
        self.WablImportances = np.ones(21)  # weights of level sets for WABL. 
    ...
    def addMF(self, var_name, *varargin, **options):
        ...
        new_mf = fl.fismf(*varargin, Name=options.get('Name'))
    ...
    def addRule(self, rule_matrix):
        for rule_def in rule_matrix:
            new_rule = fl.fisrule(rule_def, len(self.Inputs))
            self.Rules.append(new_rule)


    def __addVariable(self, in_or_out, *varargin, **options):
        new_variable = fl.fisvar(*varargin, **options)
